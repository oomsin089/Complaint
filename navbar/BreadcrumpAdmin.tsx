...
export default function ApplicantTrackingAdmin() {
  const pathname = usePathname();

  const getPaths = () => {
    const paths: TracksType[] = [
      { level: 1, name: "Dashboard", linkTo: "/adminmain" },
    ];
    ...
    const segments = pathname.split('/').filter(segment => segment && segment !== 'adminmain');
    let currentPath = '';
